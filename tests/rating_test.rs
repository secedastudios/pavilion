...
async fn setup_rated_film(app: &mut axum::Router) -> (String, String, String, String) {
    ...
    let resp = app
        ...
        .oneshot(
            Request::post("/platforms")
                .header("content-type", "application/x-www-form-urlencoded")
                .header("cookie", &cu)
                .body(Body::from("name=Rate+Channel&description=Test"))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    // Create a watch session so the curator can rate the film
    app.clone()
        .oneshot(
            Request::post(&format!("/watch/rate-channel/rated-film/heartbeat"))
                .header("content-type", "application/x-www-form-urlencoded")
                .header("cookie", &cu)
                .body(Body::from("position=60&duration=5400"))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(
            Request::post(&format!("/p/{plat_slug}/films/{film_id}/rate"))
                .header("content-type", "application/x-www-form-urlencoded")
                .header("cookie", &cu)
                .body(Body::from("score=4&review_text=Great+film"))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let resp = app
        .oneshot(
            Request::post(&format!("/p/{plat_slug}/films/{film_id}/rate"))
                .header("content-type", "application/x-www-form-urlencoded")
                .header("cookie", &cu)
                .body(Body::from("score=5&review_text=Changed+my+mind"))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(
            Request::post("/platforms")
                .header("content-type", "application/x-www-form-urlencoded")
                .header("cookie", &cu2)
                .body(Body::from("name=Other+Channel"))
                .unwrap(),
        )
        .await
        .unwrap();
