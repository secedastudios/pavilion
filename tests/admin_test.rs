...
async fn admin_requires_admin_role() {
    ...
    let resp = app.oneshot(
        Request::get("/admin").header("cookie", &cookie).body(Body::empty()).unwrap(),
    ).await.unwrap();
...
async fn showcase_loads_without_auth() {
    ...
    let resp = app.oneshot(Request::get("/showcase").body(Body::empty()).unwrap()).await.unwrap();
...
async fn security_headers_present() {
    ...
    let resp = app.oneshot(Request::get("/healthcheck").body(Body::empty()).unwrap()).await.unwrap();
