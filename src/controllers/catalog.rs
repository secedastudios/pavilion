...
pub async fn browse(
        ...
        if !q.trim().is_empty() {
            conditions.push(
                "(string::lowercase(title) CONTAINS string::lowercase($search_q) \
                 OR string::lowercase(synopsis) CONTAINS string::lowercase($search_q))".to_string()
            );
    ...
    if let Some(lang) = &params.language {
        if !lang.trim().is_empty() {
            conditions.push(
                "string::lowercase(language) = string::lowercase($lang_filter)".to_string()
            );
    ...
    conditions.push(
        "count(->licensed_via->license[WHERE active = true]) > 0".to_string()
    );

    let where_clause = conditions.join(" AND ");
    let query_str = format!(
        "SELECT * FROM film WHERE {where_clause} ORDER BY created_at DESC LIMIT 50"
    );
