...
use crate::controllers::{
    acquisitions, admin, auth, billing, catalog, dmca, enrichment, events, films, home, legal,
    licenses, payments, platforms, player, profile, ratings, revenue, settings, showcase,
    transcode, upload,
};
use crate::db::Db;
...
pub fn build_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/auth/slatehub/callback",
            get(auth::slatehub_oauth_callback),
        )
        // Legal pages
        ...
        .route(
            "/films/{id}",
            get(films::show).put(films::update).delete(films::archive),
        )
        .route("/films/{id}/edit", get(films::edit))
        .route("/films/{id}/status", post(films::update_status))
        .route(
            "/films/{id}/upload",
            post(upload::upload_film).layer(DefaultBodyLimit::max(2 * 1024 * 1024 * 1024)),
        ) // 2 GB
        .route(
            "/films/{id}/poster",
            post(upload::upload_poster).layer(DefaultBodyLimit::max(20 * 1024 * 1024)),
        ) // 20 MB
        ...
        .route(
            "/films/{id}/transcode",
            post(transcode::start_transcode).get(transcode::jobs_list),
        )
        .route(
            "/films/{film_id}/transcode/{job_id}/progress",
            get(transcode::job_progress),
        )
        // Licenses
        .route(
            "/films/{id}/licenses",
            get(licenses::index).post(licenses::create),
        )
        .route("/films/{id}/licenses/new", get(licenses::new_form))
        .route(
            "/films/{film_id}/licenses/{license_id}",
            get(licenses::edit).put(licenses::update),
        )
        .route(
            "/films/{film_id}/licenses/{license_id}/deactivate",
        ...
        .route(
            "/films/{film_id}/requests/{request_id}/approve",
            post(acquisitions::approve_request),
        )
        .route(
            "/films/{film_id}/requests/{request_id}/reject",
            post(acquisitions::reject_request),
        )
        ...
        .route(
            "/platforms/{id}",
            get(platforms::dashboard).put(platforms::update),
        )
        ...
        .route(
            "/platforms/{id}/content/{film_id}/remove",
            post(platforms::remove_film),
        )
        ...
        .route(
            "/watch/{platform_slug}/{film_slug}",
            get(player::player_page),
        )
        .route(
            "/watch/{platform_slug}/{film_slug}/manifest.m3u8",
            get(player::hls_manifest),
        )
        .route(
            "/watch/{platform_slug}/{film_slug}/manifest.mpd",
            get(player::dash_manifest),
        )
        .route(
            "/watch/{platform_slug}/{film_slug}/heartbeat",
            post(player::playhead_heartbeat),
        )
        .route("/segments/{token}", get(player::segment_proxy))
        // Ratings
        .route(
            "/p/{platform_slug}/films/{film_id}/rate",
            post(ratings::submit_rating).delete(ratings::delete_rating),
        )
        .route(
            "/p/{platform_slug}/films/{film_id}/ratings",
            get(ratings::list_ratings),
        )
        .route(
            "/p/{platform_slug}/ratings/{rating_id}/hide",
            post(ratings::hide_rating),
        )
        // Payments
        .route("/platforms/{id}/payments", get(payments::payment_settings))
        .route(
            "/platforms/{id}/payments/connect",
            post(payments::start_connect_onboarding),
        )
        .route(
            "/platforms/{id}/payments/callback",
            get(payments::connect_callback),
        )
        .route(
            "/p/{platform_slug}/checkout",
            post(payments::create_checkout),
        )
        .route("/webhooks/stripe", post(payments::stripe_webhook))
        // Revenue & Analytics
        .route("/revenue", get(revenue::filmmaker_dashboard))
        .route(
            "/platforms/{id}/analytics",
            get(revenue::platform_analytics),
        )
        // DMCA
        ...
        .route(
            "/films/{film_id}/claims/{claim_id}/counter",
            post(dmca::counter_claim),
        )
        .route("/admin/dmca/{claim_id}", post(dmca::review_claim))
        // Events
        .route(
            "/platforms/{id}/events",
            get(events::index).post(events::create),
        )
